...
CanvasRenderer = class CanvasRenderer {
  ...
  addMeteor({speed, hue, thickness, length, link}) {
    ...
    this.meteors.push({
      ...
      y: -50,
      speed: speed || this.rand(1, 3),
      hue: Math.round(hue) || this.rand(0, 360),
  ...
  createParticles(meteor) {
    if (this.particles.length < this.particleMax - this.meteors.length && meteor.thickness > 5) {
      return this.particles.push({
        ...
        radius: Math.round(this.rand(1, 6) / 2),
        alpha: this.rand(15, 30) / 100,
        hue: meteor.hue,
        light: 50
      });
    }
